This section defines <artifacts:Wallet-Relying Party Registration Certificate (WRPRC)|Wallet-Relying Party Registration Certificates (WRPRCs)>, as described in [ARF]. The <artifacts:Wallet-Relying Party Registration Certificate (WRPRC)|WRPRC> provides detailed information about the <roles:Attestation Provider (AP)|Attestation Provider>'s entitlements, the <credentials:Attestation|Attestations> they issue, and their intended use.
...
| [Attestation Provider Attributes](#attestation-provider-attributes)   | CONDITIONAL           | Attributes describing attestations issued by an <roles:Provider of Electronic Attestation of Attributes (EAA Provider)\|EAA Provider>.<br />**REQUIRED FOR:** <roles:Provider of Electronic Attestation of Attributes (EAA Provider)\|EAA Providers>. |
...
| `registry_uri`    | `string`  | REQUIRED  | URL pointing to the national registry API endpoint of the registered <roles:Wallet-Relying Party (WRP)\|WRP>. | [ETSI TS 119 475, Table 7] - `registryURI` |
| `info_uri`        | `string`  | REQUIRED  | URL general-purpose web address. | [ETSI TS 119 475, Table 7] - `infoURI` |
| `support_uri`     | `string`  | REQUIRED  | URL or email address to use in data deletion or portability requests related to the <roles:Wallet-Relying Party (WRP)\|WRP>. | [ETSI TS 119 475, Table 7] - `supportURI` |
...
| Attribute         | Type              | Presence  | Description   | Reference |
| ----------------- | :---------------: | :-------: | ------------- | --------- |
| `entitlements`    | `array[string]`   | REQUIRED  | A list of entitlement URIs assigned to the <roles:Wallet-Relying Party (WRP)\|WRP>. | [ETSI TS 119 475, Table 7] - `entitlement` |
...
!!! note "Defined Entitlements"
    ...
    | `QEAA_Provider`                   | `https://uri.etsi.org/19475/Entitlement/QEAA_Provider`                | <roles:Provider of Qualified Electronic Attestation of Attributes (QEAA Provider)\|QEAA Provider> | Annex A.2.2 |
    | `Non_Q_EAA_Provider`              | `https://uri.etsi.org/19475/Entitlement/Non_Q_EAA_Provider`           | <roles:Provider of Electronic Attestation of Attributes (EAA Provider)\|EAA Provider> | Annex A.2.3 |
    | `PUB_EAA_Provider`                | `https://uri.etsi.org/19475/Entitlement/PUB_EAA_Provider`             | <roles:Provider of Public Electronic Attestation of Attributes (PuB-EAA Provider)\|PuB-EAA Provider> | Annex A.2.4 |
    ...
    | `QCert_for_ESeal_Provider`        | `https://uri.etsi.org/19475/Entitlement/QCert_for_ESeal_Provider`     | <roles:Qualified Trust Service Provider (QTSP)\|QTSP> issuing qualified certificates for electronic seals | Annex A.2.6 |
...
| `spervisory_authority.uri`    | `string`      | REQUIRED  | The URL of web form provided by the Data Protection Authority supervising the <roles:Relying Party (RP)\|Relying Party>, which users can use to report suspicious attribute presentation requests. | [ETSI TS 119 475, Table 7] - `infoURI` |
...
| Attribute                 | Type              | Presence      | Description   | Reference |
| ------------------------- | :---------------: | :-----------: | ------------- | --------- |
| `credentials`             | `array[object]`   | CONDITIONAL   | A set of credential queries, used to request <credentials:Attestation\|Attestations> from the Wallet. The <components:EUDI Wallet> will use this information to perform an over-asking validation.<br />**REQUIRED FOR:** Service Providers. | [ETSI TS 119 475, Table 9] - `credential` |
| `credentials[].format`    | `string`          | CONDITIONAL   | Format of the <credentials:Attestation>.<br />**REQUIRED FOR:** Service Providers. | [ETSI TS 119 475, Table 9] - `format` |
...
| `credentials[].claim`     | `array[object]`   | CONDITIONAL   | Array of objects that specifies attributes in the requested <credentials:Attestation>. If claim is absent, the <artifacts:Wallet-Relying Party Registration Certificate (WRPRC)\|WRPRC> does not declare any specific attributes intended to be requested by the <roles:Wallet-Relying Party (WRP)\|WRP>.<br />**REQUIRED FOR:** Service Providers. | [ETSI TS 119 475, Table 9] - `claim` |
...
| Attribute                         | Type              | Presence      | Description   | Reference |
| --------------------------------- | :---------------: | :-----------: | ------------- | --------- |
| `provides_attestations`           | `array[object]`   | CONDITIONAL   | A set of <credentials:Attestation\|Attestations> issued by the <roles:Wallet-Relying Party (WRP)\|WRP> with <credentials:Electronic Attestation of Attributes (EAA)\|EAA> entitlements.<br />**REQUIRED FOR:** <roles:Provider of Electronic Attestation of Attributes (EAA Provider)\|EAA Providers>. | [ETSI TS 119 475, Table 8] - `providesAttestations` |
| `provides_attestations[].format`  | `string`          | CONDITIONAL   | Format of the <credentials:Attestation>.<br />**REQUIRED FOR:** <roles:Provider of Electronic Attestation of Attributes (EAA Provider)\|EAA Providers>. | [ETSI TS 119 475, Table 8] - `format` |
| `provides_attestations[].meta`    | `object`          | CONDITIONAL   | Metadata to identify the <data-elements:Attestation Type>.<br />**REQUIRED FOR:** <roles:Provider of Electronic Attestation of Attributes (EAA Provider)\|EAA Providers>. | [ETSI TS 119 475, Table 8] - `meta` |
| `provides_attestations[].claim`   | `array[object]`   | CONDITIONAL   | Objects that specifies attributes in the requested <credentials:Attestation>.<br />**REQUIRED FOR:** <roles:Provider of Electronic Attestation of Attributes (EAA Provider)\|EAA Providers> only if provided by the <components:Register>. | [ETSI TS 119 475, Table 8] - `claim` |
...
| `iat`                     | `unix_timestamp`  | REQUIRED      | Unix timestamp indicating when the <artifacts:Wallet-Relying Party Registration Certificate (WRPRC)\|WRPRC> was issued. | [ETSI TS 119 475, Table 7] - `technical` |
| `exp`                     | `unix_timestamp`  | OPTIONAL      | Expiration time of the <artifacts:Wallet-Relying Party Registration Certificate (WRPRC)\|WRPRC> as a Unix timestamp. | [ETSI TS 119 475, Table 10] - `technical` |
...
| Attribute             | Type          | Presence      | Description   | Reference |
| --------------------- | :-----------: | :-----------: | ------------- | --------- |
| `intermediary`        | `object`      | CONDITIONAL   | Used when the <roles:Wallet-Relying Party (WRP)\|WRP> operates via an <roles:Relying Party Intermediary (RPI)\|Intermediary>.<br />**REQUIRED IF:** Intermediary is used. | [ETSI TS 119 475, Table 10] - `usesIntermediary` |
| `intermediary.sub`    | `string`      | CONDITIONAL   | Identifier of the <roles:Relying Party Intermediary (RPI)\|Intermediary> as specified by the <roles:Relying Party Intermediary (RPI)\|Intermediary> <artifacts:Wallet-Relying Party Access Certificate (WRPAC)\|WRPAC>.<br />**REQUIRED IF:** <roles:Relying Party Intermediary (RPI)\|Intermediary> is used. | [ETSI TS 119 475, Table 10] - `usesIntermediary` |
| `intermediary.sname`  | `string`      | CONDITIONAL   | Common name of the <roles:Relying Party Intermediary (RPI)\|Intermediary> as specified by the <roles:Relying Party Intermediary (RPI)\|Intermediary> <artifacts:Wallet-Relying Party Access Certificate (WRPAC)\|WRPAC>.<br />**REQUIRED IF:** <roles:Relying Party Intermediary (RPI)\|Intermediary> is used. | [ETSI TS 119 475, Table 10] - `usesIntermediary` |
