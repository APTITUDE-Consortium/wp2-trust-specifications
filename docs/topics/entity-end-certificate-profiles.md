This section extends the [X.509 Certificate Profile](../sections/trust-artifacts.md#x509-certificate-profile) and specifies Certificate Profiles for Entity Sign/Seal Certificates, which are used for signing and sealing various <credentials:Attestation|Attestations>.

??? references

    To guarantee the interoperability across all entities of the <components:EUDI Wallet> ecosystem, Entity Sign/Seal Certificates should adhere to common requirements, with respect to their content and format. The technical specifications describing such content are distributed between multiple documents and for a purpose of proper referencing are listed below:
    ...
    - **ETSI TS 119 412-6** (profiles the Sign/Seal certificate for various entities in the EUDI Wallet ecosystem)
    - **ETSI TS 119 612**
    - **RFC 3986**
    - **RFC 5280**
...
!!! choice "APTITUDE Implementation Choices"
    ...
    - Entity Sign/Seal Certificates SHALL be issued by a <roles:Certificate Authority (CA)|CA> and SHALL NOT be self-signed.
...
The specific requirements for <roles:Provider of Person Identification Data (PID Provider)|PID Provider> Sign/Seal Certificates are specified in [ETSI TS 119 412-6, Clause 4].
...
!!! choice "APTITUDE Implementation Choice"
    ...
    <roles:Provider of Person Identification Data (PID Provider)|PID Provider> Sign/Seal Certificates are not Qualified Certificates. Therefore, the `qcStatements` extension SHALL NOT contain `QCStatement` structures with `statementId` set to `0.4.0.1862.1.1` (`id-etsi-qcs-QcCompliance`) or `0.4.0.1862.1.4` (`id-etsi-qcs-QcSSCD`).
...
The specific requirements for <roles:Wallet Provider (WP)|Wallet Provider> Sign/Seal Certificates are specified in [ETSI TS 119 412-6, Clause 5].
...
!!! choice "APTITUDE Implementation Choice"
    ...
    <roles:Wallet Provider (WP)|Wallet Provider> Sign/Seal Certificates are not Qualified Certificates. Therefore, the `qcStatements` extension SHALL NOT contain `QCStatement` structures with `statementId` set to `0.4.0.1862.1.1` (`id-etsi-qcs-QcCompliance`) or `0.4.0.1862.1.4` (`id-etsi-qcs-QcSSCD`).
...
The specific requirements for <roles:Provider of Electronic Attestation of Attributes (EAA Provider)|EAA Provider> and <roles:Provider of Qualified Electronic Attestation of Attributes (QEAA Provider)|QEAA Provider> Sign/Seal Certificates are specified in Clauses 6 and 7 of [ETSI TS 119 412-6], respectively.
...
| `qcStatements`            | REQUIRED (QEAA), OPTIONAL (EAA)   | For **<credentials:Qualified Electronic Attestation of Attributes (QEAA)\|QEAA>**: It SHALL contain a `QCStatement` structure with `statementId` set to `0.4.0.1862.1.1` (`id-etsi-qcs-QcCompliance`), referred to as `esi4-qcStatement-1`.<br />For **<credentials:Electronic Attestation of Attributes (EAA)\|EAA>**:<ul><li>It SHALL NOT contain a `QCStatement` structure with `statementId` set to `0.4.0.1862.1.1` (`id-etsi-qcs-QcCompliance`), referred to as `esi4-qcStatement-1`.</li><li>It SHALL NOT contain a `QCStatement` structure with `statementId` set to `0.4.0.1862.1.4` (`id-etsi-qcs-QcSSCD`), referred to as `esi4-qcStatement-4`.</li></ul>For **both**:<ul><li>It MAY contain additional `QCStatement` structures among those defined in [ETSI EN 319 412-5, Clause 4.2].</li><li>In any case, it SHALL NOT contain a `QCStatement` structure with `statementId` set to `0.4.0.1862.1.7` (`id-etsi-qcs-QcCClegislation`), referred to as `esi4-qcStatement-7`.</li></ul> |

!!! choice "APTITUDE Implementation Choices"

    - <roles:Provider of Qualified Electronic Attestation of Attributes (QEAA Provider)|QEAA Provider> Sign/Seal Certificates SHALL be Qualified Certificates. Therefore, the `qcStatements` extension SHALL contain at least a `QCStatement` structure with `statementId` set to `0.4.0.1862.1.1` (`id-etsi-qcs-QcCompliance`).
    
    - <roles:Provider of Electronic Attestation of Attributes (EAA Provider)|EAA Provider> Sign/Seal Certificates SHALL NOT be Qualified Certificates. Therefore, the `qcStatements` extension SHALL NOT contain `QCStatement` structures with `statementId` set to `0.4.0.1862.1.1` (`id-etsi-qcs-QcCompliance`) or `0.4.0.1862.1.4` (`id-etsi-qcs-QcSSCD`).
...
!!! warning
    ...
    While the specific requirements for <roles:Provider of Public Electronic Attestation of Attributes (PuB-EAA Provider)|PuB-EAA Provider> Sign/Seal Certificates specified in [ETSI TS 119 412-6, Clause 8] do not require these certificates to be qualified, [EU REG 2024/1183, Art. 45f(1)(b)] requires <credentials:Public Electronic Attestation of Attributes (PuB-EAA)|PuB-EAA> to be signed with a qualified certificate.

!!! choice "APTITUDE Implementation Choice"

    To comply with [EU REG 2024/1183, Art. 45f(1)(b)], although not explicitly required by [ARF] or [ETSI TS 119 412-6], <roles:Provider of Public Electronic Attestation of Attributes (PuB-EAA Provider)|PuB-EAA Provider> Sign/Seal Certificates are Qualified Certificates. Therefore, the `qcStatements` extension SHALL contain at least a `QCStatement` structure with `statementId` set to `0.4.0.1862.1.1` (`id-etsi-qcs-QcCompliance`).
