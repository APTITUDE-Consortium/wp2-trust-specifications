...
!!! choice "APTITUDE Implementation Choices"
    ...
    - The <artifacts:Official Journal of APTITUDE (OJA)> SHALL publish a fixed location and fixed authorized signing certificate set for each <artifacts:List of Trusted Entities (LoTE)|LoTE> type. The publication URI, type-specific locations, and certificate sets SHALL remain unchanged during the pilot.
...
!!! choice "APTITUDE Pilot OJA Publication"

    The <artifacts:Official Journal of APTITUDE (OJA)|OJA> publication SHALL remain at a fixed URI for the duration of the pilot. It is not signed or independently authenticated. <artifacts:List of Trusted Entities (LoTE)|LoTE> consumers accept its type-specific locations and signing certificate sets as pilot discovery data. A successful comparison with that certificate set does not independently authenticate the <artifacts:Official Journal of APTITUDE (OJA)|OJA> publication.
...
The `x5c`, `kid`, and `x5u` header parameters SHALL NOT be used in the pilot profile. A validator SHALL select the signing certificate only from the type-specific <artifacts:Official Journal of APTITUDE (OJA)|OJA> certificate set, using `x5t#S256`.
...
??? example "Example: PID Provider <artifacts:List of Trusted Entities (LoTE)|LoTE>"
...
The QEAA Provider and EAA Provider `LoTEType`, `StatusDeterminationApproach`, `SchemeTypeCommunityRules`, and `ServiceTypeIdentifier` URIs are APTITUDE-defined identifiers documented in [APTITUDE LoTE identifiers](../lote-identifiers.md); the other type-specific values below are the ETSI profile values.
...
| `DistributionPoints`              | `Array`       | PROHIBITED | The fixed endpoint is supplied by the <artifacts:Official Journal of APTITUDE (OJA)\|OJA> and SHALL NOT be duplicated in the list. | [ETSI TS 119 602, Clause 6.3.16], This Specification |
| `SchemeExtensions`                | `Array`       | PROHIBITED | The pilot does not use scheme extensions. | [ETSI TS 119 602, Clause 6.3.17], This Specification |

!!! choice "APTITUDE Pilot History and Publication"

    - The pilot SHALL publish only the current <artifacts:List of Trusted Entities (LoTE)|LoTE> version for each type at its fixed endpoint.
...
| `TEInformationExtensions`         | `Array`       | PROHIBITED | The pilot does not use trusted entity information extensions. | [ETSI TS 119 602, Clause 6.5.5], This Specification |
...
| `TEServiceDefinitionURI`          | `Array`       | OPTIONAL | No additional requirements. | [ETSI TS 119 602, Clause 6.6.8], This Specification |
...
!!! choice "APTITUDE Implementation Choices"
    ...
    For the pilot-specific <roles:Provider of Qualified Electronic Attestation of Attributes (QEAA Provider)|QEAA Provider> and <roles:Provider of Electronic Attestation of Attributes (EAA Provider)|EAA Provider> lists, `ServiceTypeIdentifier` SHALL respectively use `http://aptitude-consortium/SvcType/{Specific_attestation}/Issuance`, `http://aptitude-consortium/SvcType/{Specific_attestation}/Revocation` as applicable.
