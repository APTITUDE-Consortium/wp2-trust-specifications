...
| ------------------------- | ------------- | ----- |
| `authorityKeyIdentifier`  | CONDITIONAL   | **REQUIRED IF:** the certificate is not self-signed. For self-signed certificates, it is RECOMMENDED. If the extension is present, the `keyIdentifier` field SHOULD be derived from the public key using the methods defined in [RFC 5280, Section 4.2.1.1]. |
| `subjectKeyIdentifier`    | REQUIRED      | Provides a key identifier for the Trust Anchor public key. The `keyIdentifier` field SHOULD be derived from the subject public key using the methods defined in [RFC 5280, Section 4.2.1.2]. This extension SHOULD support reliable Certificate Path construction and certificate matching in LoTE- or TL-based deployments. |
