...
This section describes the Onboarding Process within APTITUDE, where it is realised as the mocked-up version of the WRP Registration Process and Notification Process (defined in [Trust Architecture](#4-trust-architecture)) through which entities become operational and recognisable in the common trust infrastructure of the pilot.
...
- **Trust infrastructure entities** (in short, *infrastructure entities*) are the entities that operate the trust infrastructure through which onboarding is performed, namely registration (the <roles:Registrar>, operating the <components:Register>), certificate issuance (the <roles:Provider of Wallet Relying Party Access Certificate (Provider of WRPAC)|Provider of WRPAC> and the <roles:Provider of Wallet Relying Party Registration Certificate (Provider of WRPRC)|Provider of WRPRC>), and publication of the trusted lists (the LoTE / Trusted List Provider). The LoTE / Trusted List Provider has a special role, as its own trust anchor is the root against which the lists are validated.
...
graph LR
    ...
    subgraph LoTEs["Infrastructure LoTEs"]
...
2. **List trust anchor.** The self-signed root of the LoTE / Trusted List Provider is the trust anchor used to validate the lists. In the normative framework this anchor is the certificate published in the <artifacts:Official Journal of the European Union (OJEU)|OJEU>; within APTITUDE the OJEU publication is mocked and the certificate is self-signed. The distribution point where this trust anchor is made available to the APTITUDE participants is an operational matter, out of scope of this document.
3. **Notification of trust anchors.** The trust anchor of each infrastructure entity is listed in its corresponding <artifacts:List of Trusted Entities (LoTE)|LoTE> (the Registrars LoTE, the Providers of WRPAC LoTE, or the Providers of WRPRC LoTE).
4. **Signing and publication.** The LoTE / Trusted List Provider signs/seals the LoTEs and publishes them at a distribution point referenced by the mocked OJEU/LOTL, so that they can be retrieved at validation time (see [Trust Anchor Validation Process](#trust-anchor-validation-process)).

The same LoTE / Trusted List Provider also signs the lists populated during operational entity onboarding (the PID Providers, PuB-EAA Providers, and Wallet Providers LoTEs, and the <artifacts:EU Member State Trusted List (EUMS TL)|EUMS TL> referenced by the <artifacts:List Of Trusted Lists (LOTL)>).
...
- the trusted-list entry (<artifacts:List of Trusted Entities (LoTE)|LoTE>, <artifacts:EU Member State Trusted List (EUMS TL)|EUMS TL>, or the trusted list for non-qualified EAA Providers), produced by the [Notification and Publication](#notification-and-publication) step.
...
- the <roles:Wallet Provider (WP)|Wallet Provider> follows a notification-only path, with no <components:Register> record and no <artifacts:Wallet-Relying Party Access Certificate (WRPAC)|WRPAC>, and its trust anchor is published in the Wallet Providers <artifacts:List of Trusted Entities (LoTE)|LoTE>;
