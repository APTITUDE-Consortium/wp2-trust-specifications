...
- `REMOVED`: Indicates the revocation of an entity's `REGISTERED` status due to voluntary offboarding, a severe security breach, or a critical compliance failure.
    ...
    - **Historical Operations**: Ecosystem participants MAY validate historical data, signatures, and attestations generated prior to the Removal timestamp where the evidence required for historical validation remains available.

!!! choice "APTITUDE Implementation Choices"

    The following historical-information rules SHALL apply:

    - For <roles:Provider of Person Identification Data (PID Provider)|PID Providers>, <roles:Provider of Qualified Electronic Attestation of Attributes (QEAA Provider)|QEAA Providers>, <roles:Provider of Electronic Attestation of Attributes (EAA Provider)|EAA Providers>, <roles:Wallet Provider (WP)|Wallet Providers>, <roles:Provider of Wallet-Relying Party Access Certificate (Provider of WRPAC)|Providers of WRPAC>, <roles:Provider of Wallet-Relying Party Registration Certificate (Provider of WRPRC)|Providers of WRPRC>, and <roles:Registrar|Registrars> and <components:Register|Registers> <artifacts:List of Trusted Entities (LoTE)|LoTE>, the current list SHALL NOT contain historical keys or service states, and the <roles:List of Trusted Entities Provider (LoTE Provider)|LoTE Provider> SHALL NOT publish previous list instances for historical membership validation. <roles:Provider of Qualified Electronic Attestation of Attributes (QEAA Provider)|QEAA Provider> and <roles:Provider of Electronic Attestation of Attributes (EAA Provider)|EAA Provider> <artifacts:List of Trusted Entities (LoTE)|LoTE> SHALL follow the <roles:Provider of Person Identification Data (PID Provider)|PID Provider> structure and management for this purpose.
    - For <roles:Provider of Public Electronic Attestation of Attributes (PuB-EAA Provider)|Pub-EAA Providers> <artifacts:List of Trusted Entities (LoTE)|LoTE>, withdrawn services SHALL remain in the current list with `ServiceStatus` set to `withdrawn`, and prior status records SHALL be retained in `ServiceHistory`. A `ServiceHistoryInstance` SHALL identify its former key by `X509SKI` and SHALL NOT contain an `X509Certificate`. `HistoricalInformationPeriod` SHALL be `65535`, and `StatusStartingTime` SHALL record when the current status applies. Trust decisions within APTITUDE SHALL use only currently `notified` issuance services.
...
Depending on the circumstances, an entity in the `REMOVED` state MAY have its <artifacts:Electronic Signature\|Signature>/<artifacts:Electronic Seal\|Seal> certificates revoked. Historical validation of artifacts it issued depends on the continued availability of the required evidence; non-PuB-EAA <artifacts:List of Trusted Entities (LoTE)\|LoTE> history is not published within the APTITUDE ecosystem. Further details are in the [Operational Effects of Removal](#operational-effects-of-removal) section.
...
- For <artifacts:List of Trusted Entities (LoTE)|LoTE>, only the current version is published. Publication of a new signed version at the same fixed endpoint replaces the preceding version. Earlier non-PuB-EAA versions are not published for historical membership validation. Historical information for <roles:Provider of Public Electronic Attestation of Attributes (PuB-EAA Provider)|PuB-EAA Provider> services remains in `ServiceHistory` within the current list (see [Trust Anchor Validation Process](../sections/trust-evaluation-process.md#trust-anchor-validation-process)).
...
stateDiagram-v2
    ...
    state "LoTE" as LoTEGroup {
        ...
        LoTE_CURRENT --> LoTE_CURRENT : New signed version at fixed endpoint
    }
...
| State | Definition | Applicable Artifacts | Technical Mean |
| :--- | :--- | :--- | :--- |
| `CURRENT` | Indicates the latest <artifacts:List of Trusted Entities (LoTE)\|LoTE> version published at the fixed endpoint. The preceding version is replaced and SHALL NOT be used for operational trust decisions. | <artifacts:List of Trusted Entities (LoTE)\|LoTE>. | Reissuance at the fixed endpoint with an authorized signing certificate; validator refresh and sequence checks; `ServiceHistory` retained within the current PuB-EAA list. Earlier non-PuB-EAA versions are not published. |
...
- **Technical Configurations Updates**:
    - **<artifacts:Electronic Signature|Signature>/<artifacts:Electronic Seal|Seal> Key update**: the <roles:Wallet-Relying Party (WRP)|WRP> SHALL notify <artifacts:Electronic Signature|Signature>/<artifacts:Electronic Seal|Seal> Key updates to the <roles:Certificate Authority (CA)|Certificate Authority> responsible for the issuance of these certificates.
        ...
        - **<artifacts:List of Trusted Entities (LoTE)|LoTE> Update** [for a certificate used as a LoTE <artifacts:Trust Anchor>]: Upon obtaining a new <artifacts:Electronic Signature|Signature>/<artifacts:Electronic Seal|Seal> certificate with the updated key, an entity listed in a dedicated <artifacts:List of Trusted Entities (LoTE)|LoTE> SHALL notify the <roles:List of Trusted Entities Provider (LoTE Provider)|LoTE Provider>, which SHALL publish a new version of that <artifacts:List of Trusted Entities (LoTE)|LoTE> with the updated `ServiceInformation` component at the fixed endpoint using an authorized signing certificate.
...
- **Technical Configurations Updates**:
    - **<artifacts:Electronic Signature|Signature>/<artifacts:Electronic Seal|Seal> Key update**: the entity SHALL notify the <artifacts:Electronic Signature|Signature>/<artifacts:Electronic Seal|Seal> Key updates to the <roles:Certificate Authority (CA)|Certificate Authority> responsible for the issuance of these certificates.
        - **<artifacts:Electronic Signature|Signature>/<artifacts:Electronic Seal|Seal> Certificate Revocation**: The <roles:Certificate Authority (CA)|Certificate Authority> SHALL revoke the entity's current <artifacts:Electronic Signature|Signature>/<artifacts:Electronic Seal|Seal> certificate.
        - **<artifacts:Electronic Signature|Signature>/<artifacts:Electronic Seal|Seal> Certificate Re-issuance**: The <roles:Certificate Authority (CA)|Certificate Authority> SHALL issue a new <artifacts:Electronic Signature|Signature>/<artifacts:Electronic Seal|Seal> certificate with the updated key.
        - **<artifacts:List of Trusted Entities (LoTE)|LoTE> Update** [for a certificate used as a <artifacts:List of Trusted Entities (LoTE)|LoTE> <artifacts:Trust Anchor>]: Upon obtaining a new <artifacts:Electronic Signature|Signature>/<artifacts:Electronic Seal|Seal> certificate with the updated key, the entity SHALL notify the <roles:List of Trusted Entities Provider (LoTE Provider)|LoTE Provider>, which SHALL publish a new version of the applicable <artifacts:List of Trusted Entities (LoTE)|LoTE> with the updated `ServiceInformation` component at the applicable endpoint using an authorized signing certificate.
    - **Endpoints Update**: When an endpoint published in an applicable <artifacts:List of Trusted Entities (LoTE)|LoTE> is updated, the entity SHALL notify the <roles:List of Trusted Entities Provider (LoTE Provider)|LoTE Provider>, which SHALL publish a new version of that <artifacts:List of Trusted Entities (LoTE)|LoTE> with the updated `ServiceInformation` component at the applicable endpoint using an authorized signing certificate.
...
- **<artifacts:Wallet-Relying Party Registration Certificate (WRPRC)|WRPRC> Revocation**: The <roles:Provider of Wallet-Relying Party Registration Certificate (Provider of WRPRC)|Provider of WRPRC> SHALL revoke the entity's current <artifacts:Wallet-Relying Party Registration Certificate (WRPRC)|WRPRC> if present. Revocation SHALL be executed by setting the status value of the <artifacts:Wallet-Relying Party Registration Certificate (WRPRC)|WRPRC> within the corresponding <artifacts:Status List Token> to `0x01`.
- **<artifacts:Electronic Signature|Signature>/<artifacts:Electronic Seal|Seal> Certificate Revocation**: The <roles:Certificate Authority (CA)|Certificate Authority> SHALL revoke the entity's current <artifacts:Electronic Signature|Signature>/<artifacts:Electronic Seal|Seal> certificate.
- **<artifacts:List of Trusted Entities (LoTE)|LoTE> Update** [for a LoTE-listed <roles:Provider of Person Identification Data (PID Provider)|PID Provider>, <roles:Provider of Qualified Electronic Attestation of Attributes (QEAA Provider)|QEAA Provider>, <roles:Provider of Electronic Attestation of Attributes (EAA Provider)|EAA Provider>, or <roles:Provider of Public Electronic Attestation of Attributes (PuB-EAA Provider)|Pub-EAA Provider>]: the <roles:Wallet-Relying Party (WRP)|WRP> SHALL notify the <roles:List of Trusted Entities Provider (LoTE Provider)|LoTE Provider>, which SHALL publish a new version of the applicable <artifacts:List of Trusted Entities (LoTE)|LoTE> at the applicable endpoint using an authorized signing certificate.
    - For a <roles:Provider of Public Electronic Attestation of Attributes (PuB-EAA Provider)|Pub-EAA Provider>, the applicable <artifacts:List of Trusted Entities (LoTE)|LoTE> SHALL keep the service in the current list with `ServiceStatus` set to `withdrawn`, set `StatusStartingTime` to the time at which the withdrawn status takes effect, and retain its prior status record in `ServiceHistory` using `X509SKI` without an `X509Certificate`. The withdrawn service SHALL NOT supply an operational <artifacts:Trust Anchor>.
    - For the <roles:Provider of Person Identification Data (PID Provider)|PID Provider>, <roles:Provider of Qualified Electronic Attestation of Attributes (QEAA Provider)|QEAA Provider>, and <roles:Provider of Electronic Attestation of Attributes (EAA Provider)|EAA Provider> <artifacts:List of Trusted Entities (LoTE)|LoTE>, the entity SHALL be removed from the current list; the current list SHALL NOT contain historical keys or service states, and the <roles:List of Trusted Entities Provider (LoTE Provider)|LoTE Provider> SHALL NOT publish previous list instances for historical validation.
...
- **<artifacts:List of Trusted Entities (LoTE)|LoTE> Update**: the Trust Artifact Provider SHALL notify the <roles:List of Trusted Entities Provider (LoTE Provider)|LoTE Provider>, which SHALL publish a new version of the applicable <artifacts:List of Trusted Entities (LoTE)|LoTE> at its applicable endpoint using an authorized signing certificate and remove the entity from the current list. The current list SHALL NOT contain historical keys or service states, and the <roles:List of Trusted Entities Provider (LoTE Provider)|LoTE Provider> SHALL NOT publish previous list instances for historical validation.
