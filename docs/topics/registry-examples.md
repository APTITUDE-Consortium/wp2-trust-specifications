...
This file contains non-normative JSON examples complementing the two baseline examples in [Wallet-Relying Party Registration Certificate](../sections/trust-artifacts.md#wallet-relying-party-registration-certificate) (§11). Each example illustrates a different registrant profile. Where meaningful, both the registration view (full, includes `postalAddress`) and the published API view (excludes `postalAddress`) are shown.
